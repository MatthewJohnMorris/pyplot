...
class Node(object):
    ...
    def __repr__(self):
        return f"Node({self.data}, #{self.index})"
...
def create(point_and_index_list=None, dimensions=None, axis=0, sel_axis=None):
    ...
    if not point_and_index_list and not dimensions:
        raise ValueError('either point_list or dimensions must be provided')

    elif point_and_index_list:
        dimensions = check_dimensionality(point_and_index_list, dimensions)

    # by default cycle through the axis
    sel_axis = sel_axis or (lambda prev_axis: (prev_axis+1) % dimensions)

    if not point_and_index_list:
        return KDNode(sel_axis=sel_axis, axis=axis, dimensions=dimensions)

    # Sort point list and choose median as pivot element
    point_and_index_list = list(point_and_index_list)
    point_and_index_list.sort(key=lambda point_and_index: point_and_index[0][axis])
    median = len(point_and_index_list) // 2

    loc   = point_and_index_list[median]
    left  = create(point_and_index_list[:median], dimensions, sel_axis(axis))
    right = create(point_and_index_list[median + 1:], dimensions, sel_axis(axis))
    #print(f"writing {loc} at {median+start_ix}")
    return KDNode(loc[0], left, right, axis=axis, sel_axis=sel_axis, dimensions=dimensions, index=loc[1])


def check_dimensionality(point_and_index_list, dimensions=None):
    dimensions = dimensions or len(point_and_index_list[0][0])
    for p in point_and_index_list:
        if len(p[0]) != dimensions:
